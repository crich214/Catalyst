...
def rich_alpha_score(opportunity, conviction, execution, risk):
    score = opportunity * 0.45 + conviction * 0.30 + execution * 0.20 - risk * 0.10 + 10
    return round(clamp(score), 1)

def recommendation(stock, rich_alpha, conviction):
    if stock["risk"] >= 85:
        return "SPECULATIVE / WATCH ONLY"
    if "Moonshot" in stock["category"]:
        return "SPECULATIVE STARTER" if rich_alpha >= 75 else "WATCH"
    if rich_alpha >= 88 and conviction >= 75:
        return "BUY"
    if rich_alpha >= 78:
        return "ACCUMULATE"
    if rich_alpha >= 65:
...
def accumulation_plan(stock, recommendation_text):
    ...
    return {
        "strategy": "accumulate" if recommendation_text in ["BUY", "ACCUMULATE"] else "watch",
        "current_price": price,
        "intrinsic_value": intrinsic,
        "estimated_margin_of_safety_pct": round(((intrinsic - price) / intrinsic) * 100, 1) if intrinsic else None,
        "max_position": f"{round(max_position * 100, 1)}%",
        "ladder": [
            {
                "price_zone": f"Below ${round(intrinsic * 0.90, 2)}",
                "action": "Begin position",
                "target_position": f"{round(max_position * 0.25 * 100, 1)}%"
            },
            {
                "price_zone": f"Below ${round(intrinsic * 0.80, 2)}",
                "action": "Accumulate",
                "target_position": f"{round(max_position * 0.50 * 100, 1)}%"
            },
            {
                "price_zone": f"Below ${round(intrinsic * 0.70, 2)}",
                "action": "Aggressive accumulate if thesis unchanged",
                "target_position": f"{round(max_position * 0.75 * 100, 1)}%"
            },
            {
...
def score_stock(stock):
    ...
    rich_alpha = rich_alpha_score(opportunity, conviction, execution, stock["risk"])
    ...
    return {
        ...
        "price_decline_pct": round(stock.get("price_decline_pct", 0), 1),
