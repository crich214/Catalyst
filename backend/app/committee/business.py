...
def business_review(scored: dict, information_briefing=None):
    score = int(
    ...
    if valuation >= 70:
        positives.append("Valuation appears attractive relative to intrinsic value.")

    if conviction >= 70:
        positives.append("Business quality and conviction remain strong.")

    if execution < 60:
        concerns.append("Execution metrics remain below preferred threshold.")

    if risk >= 65:
        concerns.append("Underlying business risk has increased.")

    information_items = []
    if information_briefing:
        information_items = [
            item for item in information_briefing.items
            if "Business" in item.affected_domains
        ]

    for item in information_items:
        if item.materiality == "High":
            concerns.append(
                f"Material business information identified: {item.summary}"
            )
